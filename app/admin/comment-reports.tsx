// app/admin/comment-reports.tsx
// Admin: reported review comments, grouped by comment. Delete the comment, or
// dismiss the report(s) if the content is fine. Mirrors app/admin/reports.tsx.
...
import { deleteComment, getCommentById } from "@/services/reviewComments";
...
export default function AdminCommentReports() {
  ...
  const load = useCallback(async () => {
    try {
      const { items: reports } = await listCommentReports();
      ...
      const built: ReportGroup[] = commentIds.map((id, i) => {
        ...
        return {
          commentId: id,
          // Fall back to the report's reviewId when the comment is gone.
          reviewId: comment?.reviewId ?? reps[0]?.reviewId ?? "",
          reports: reps,
          comment,
  ...
  const dismiss = useCallback(async (group: ReportGroup) => {
    ...
    try {
      await Promise.all(group.reports.map((r) => deleteCommentReport(r.id)));
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            setBusyId(group.commentId);
            try {
              await deleteComment(group.commentId, group.reviewId);
              await Promise.all(
                group.reports.map((r) => deleteCommentReport(r.id)),
              );
              setGroups((prev) =>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <AdminHeader title="Comment reports" />
      {loading ? (
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              ...
              <Text style={styles.emptyBody}>
                Reported comments will show up here.
              </Text>
            </View>
          }
        />
